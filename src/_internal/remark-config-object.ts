 ...
 * Shared helpers for reading and rewriting Remark config objects.
 */
...
const configBaseNamePattern =
    /^(?:remark\.config|\.remarkrc)\.(?:[cm]?js|[cm]?ts)$/v;

/**
 * Check whether a filename looks like a Remark config module.
 *
 * @param filename - Absolute or relative filename.
 *
 * @returns `true` when the file name matches supported Remark config names.
 */
export const isRemarkConfigFile = (filename: string): boolean =>
 ...
 */
export const getExportedRemarkConfigObject = (
