...
const remarkConfigMetadataByName = {
    all: { icon: "🟣" },
    configuration: { icon: "🔧" },
    recommended: { icon: "🟡" },
    remarkOnly: { icon: "📝" },
...
const remarkConfigNamesByReadmeOrder = [
    "recommended",
    "remarkOnly",
    "configuration",
    "all",
];
const remarkConfigReferenceToName = {
...
const presetDocsByName = {
    ...
    remarkOnly: {
        href: "./docs/rules/presets/remark-only.md",
...
const toPresetLegendLine = (presetName) => {
    ...
    const icon = remarkConfigMetadataByName[presetName]?.icon;
...
const normalizeConfigNames = (configs) => {
    ...
    for (const value of values) {
        ...
        if (Object.hasOwn(remarkConfigReferenceToName, value)) {
            const referenceKey =
                /** @type {keyof typeof remarkConfigReferenceToName} */ (value);

            normalized.push(remarkConfigReferenceToName[referenceKey]);
        }
    }
...
const getPresetIndicator = (ruleModule) => {
    ...
    for (const configName of remarkConfigNamesByReadmeOrder) {
        if (!configNames.has(configName)) {
            continue;
        }

        const metadataKey =
            /** @type {keyof typeof remarkConfigMetadataByName} */ (configName);
        const presetLink = /** @type {keyof typeof presetDocsByName} */ (
            configName
        );

        icons.push(
            `[${remarkConfigMetadataByName[metadataKey].icon}](${presetDocsByName[presetLink].href})`
        );
    }
...
export const generateReadmeRulesSectionFromRules = (rules) => {
    ...
    return [
        ...
        "",
        "Preset key legend:",
        ...remarkConfigNamesByReadmeOrder.map(
            /** @param {keyof typeof presetDocsByName} presetName */
